...
namespace ft
{
	...
	class SetReverseIterator
	{
		...
		SetReverseIterator(): _ptr(NULL), _last_ptr(NULL), _first_ptr(NULL), _is_end(0) {}
		...
		SetReverseIterator(const nodePointer ptr): _ptr(ptr), _last_ptr(NULL), _first_ptr(NULL), _is_end(0) {}

		SetReverseIterator(const nodePointer ptr, const nodePointer _begin, const nodePointer _end): _ptr(ptr), _is_end(0)
		{
			_last_ptr = _end;
			_first_ptr = _begin;
		}

		SetReverseIterator(const nodePointer _begin, const nodePointer _end): _ptr(NULL), _is_end(0)
		{
			_last_ptr = _end;
			_first_ptr = _begin;
		}

		SetReverseIterator(const SetReverseIterator<key_type, false> &cp)
		{
			if (cp.getIsEnd() == 1)
			{
		...
		SetReverseIterator(const SetReverseIterator<key_type, true> &cp)
		{
			if (cp.getIsEnd() == 1)
			{
				nodePointer new_node = _alloc_node.allocate(1);
		...
		SetReverseIterator &operator=(const SetReverseIterator<key_type, false> &cp)
		{
			if (_is_end == 1)
			{
				_alloc_node.destroy(_ptr);
		...
		SetReverseIterator &operator=(const SetReverseIterator<key_type, true> &cp)
		{
			if (_is_end == 1)
			{
				_alloc_node.destroy(_ptr);
		...
		bool operator==(const SetReverseIterator &cp) const
		{
		...
		bool operator!=(const SetReverseIterator &cp) const { return !(*this == cp); }
		nodePointer getPointer() const { return _ptr; }
        SetReverseIterator operator++(int)
		{
			SetReverseIterator tmp;
			tmp.setEnd(0);
			tmp = *this;
			++(*this);
			return tmp;
		...
		SetReverseIterator& operator++() { getPrevNode(); return *this; }
		SetReverseIterator& operator--() { getNextNode(); return *this; }
