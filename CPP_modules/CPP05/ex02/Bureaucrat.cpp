...
{
	if (formulaire.getSignGrade() >= this->getGrade())
	{
		if (formulaire.getIsSigned() == 0)
	...
	else
	{
		...
		std::cout << formulaire.getName() << " because his grade is too low" << std::endl;
	}
}

void Bureaucrat::executeForm(Form const & form) const
{
	if (form.getSignExec() >= this->getGrade())
		form.execute(*this);
	else
	{
		std::cout << this->getName() << " couldn't execute ";
		std::cout << form.getName() << " because his grade is too low" << std::endl;
	}
}
